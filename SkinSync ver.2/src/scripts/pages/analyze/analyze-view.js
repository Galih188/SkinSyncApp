class AnalyzeView {
  ...
  _setupButtonInteractions() {
    ...
    buttons.forEach((btn) => {
      // Efek hover sudah ditangani oleh main.css, ini sebagai contoh jika ingin kustomisasi lebih
      btn.style.transition =
        "transform 0.2s ease-out, box-shadow 0.2s ease-out";
    });
  }

  // === Metode untuk memanipulasi DOM === (Tidak ada perubahan di bawah ini)
  ...
  displayAnalysisResult(result) {
    const { type, confidence, description, recommendations } = result;
    this._getElement("#analysis-result").innerHTML = `
      <div class="result-success" style="opacity:0; transform: translateY(20px); transition: all 0.5s ease-out;">
        ...
        <div class="result-actions">
          ...
          <button class="save-btn"><i data-feather="bookmark"></i> Simpan Hasil</button>
        </div>
      </div>
    `;
    this._getElement("#analyze-btn").disabled = false;
    ...
    feather.replace();
    this._setupButtonInteractions(); // Re-apply for new buttons
  }
